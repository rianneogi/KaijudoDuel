...
void DuelInterface::render(Camera& camera, const Vector2i& mousePos)
{
	//render zones
	for (int i = 0; i < 2; i++)
	{
		...
		duel.decks[i].renderCards(myPlayer);
		duel.hands[i].renderCards(myPlayer);
		duel.manazones[i].renderCards(myPlayer);
		duel.graveyards[i].renderCards(myPlayer);
		duel.shields[i].renderCards(myPlayer);
		duel.battlezones[i].renderCards(myPlayer);
	}
...
{
	duel.parseMessages(deltatime);
}

void DuelInterface::update(int deltaTime, Camera& camera, const Vector2i& mousePos)
{
	int hoverCardId = -1;
	if (mousePos.x >= 0 && mousePos.y >= 0)
	{
		glm::mat4 view, proj, projview;
		camera.render(view, proj);
		projview = proj*view;
		Vector2i screendim(SCREEN_WIDTH, SCREEN_HEIGHT);
		float minDepth = 1;
		for (int i = duel.hands[0].cards.size() - 1;i >= 0;i--)
		{
			if (duel.hands[0].cards[i]->rayTrace(mousePos, projview, screendim))
			{
				if (hoverCardId != duel.hands[0].cards[i]->UniqueId)
				{
					hoverCardId = duel.hands[0].cards[i]->UniqueId;
		...
		if (hoverCardId != -1)
		{
			printf("Intersect: %s\n", gCardNames[duel.CardList[hoverCardId]->CardId].c_str());
		}
	}
	duel.hands[0].update(camera, hoverCardId); //update hand

	for (int i = 0;i < duel.CardList.size();i++) //update cards
	{
		duel.CardList[i]->update(deltaTime);
	}
}
